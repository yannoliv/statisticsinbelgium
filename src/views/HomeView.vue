...
async function submitModalForm(event) {
  const newLine = {
    ...
    salary_bruto: parseFloat(formData.salary_bruto),
    salary_netto: parseFloat(formData.salary_netto),
    car_model: formData.car_model,
    experience: parseFloat(formData.experience),
    happy: formData.happy.value.mood,
  };
  console.log(newLine);
  const { iData, iError } = await supabase.from("Salary").insert([newLine]);
      ...
      <div class="mx-auto max-w-7xl py-16 px-4 sm:py-24 sm:px-6 lg:px-8">
        <div class="text-center">
          ...
          <p class="mx-auto mt-5 max-w-xl text-xl text-gray-500">
            By comparing wages you can be more confident if asking for a raise
            or during solicitations because of this data.
          </p>
        </div>
      </div>
    </div>

    <!-- Graph of all the salary data based on experience and bruto -->
      ...
      <div class="mt-8 flex flex-col">
        <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          ...
          >
            ...
            >
              <table
                class="w-full flex flex-row flex-no-wrap overflow-hidden divide-y divide-gray-300"
              >
                <thead class="bg-gray-50">
                  <tr
                    v-for="person in sortedData"
                    :key="person.id"
                    class="flex flex-col flex-no wrap sm:table-row py-5"
                  >
                    <th
                      scope="col"
                    ...
                    <th
                      scope="col"
                      class="sortable px-3 py-4 text-left text-sm font-semibold text-gray-900"
                    >
                      Bruto
                    </th>
                    <th
                ...
                <tbody
                  class="flex-1 sm:flex-none divide-y divide-gray-200 bg-white"
                >
                  <tr
                    v-for="person in sortedData"
                    :key="person.id"
          ...
          >
            ...
            >
              ...
              >
                <DialogTitle
                  as="h3"
                  class="text-lg font-medium leading-10 text-gray-900"
                >
                  Fill in your salary
                </DialogTitle>
                <div class="mt-2">
                  <form class="space-y-6" v-on:submit.prevent="submitModalForm">
                    ...
                    <InputField
                      label="car"
                      text="Car info"
                      placeholder="Volkswagen Golf 2019"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <RadioGroup v-model="selectedMood">
                          ...
                          <div
                            class="mt-2 flex items-center space-x-3 flex-wrap"
                          >
                            <RadioGroupOption
                              as="template"
                              v-for="mood in moods"
                            ...
                            >
                              ...
                              >
                                <span
                                  aria-hidden="true"
                                  class="bg-white h-6 w-6 rounded-full"
                                >
                                  {{ mood.mood }}
                                </span>
                              </div>
              ...
              >
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    Your salary has been succesfully submitted, Thank you for
                    participating!
                  </p>
                </div>
